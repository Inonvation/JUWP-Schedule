...
- 开水卡下方：**快捷方式行**（默认显示，可在设置关；规格见 §3.8）
...
- 显示设置（眼睛图标）→ **课表页内覆盖面板**（早期曾是独立子页与半屏弹层）：
  ...
  **布局尺寸**：`时间轴宽度`（28–72dp，默认 48dp）、`表头高度`（32–64dp，默认 44dp）——
  二者参与自适应行高计算（表头变高网格变矮），列宽 =（总宽−时间轴宽度）/列数；
...
| 条目 | 格位 | 默认 dp | 内容 |
|------|------|---------|------|
| 水贝贝 · 2×2 | 2 宽 × 2 高 | 110×110 | 只显示焦点课（正在上课 / 下一节）：课名 + 时刻 + 地点 |
| 水贝贝 · 2×4 | 2 宽 × 4 高 | 110×250 | 焦点课 + 今日剩余列表（最多 3 条） |
| 水贝贝 · 4×4 | 4 宽 × 4 高 | 250×250 | 焦点卡（含进度条）+ 完整剩余列表 + 明日预告 |

三条目共用同一套渲染与同一份状态推导，仅按 `LocalSize` 分档控制信息密度
（`SizeMode.Responsive` 声明 2×2 / 2×4 / 4×4 / 4×2 四档，用户拉伸到横条时也走 4×2 档）。
...
给常用第三方入口一排快捷 chips，放今日页列表尾部**一键开水卡下方**（无开水卡时仍在列表尾部）。
...
- 形态：横向滚动 chips（图标 + 名称），超出屏宽横滑；圆角对齐设置卡 14dp。
- 预设 3 个：拼多多取件码（scheme 直达取件页）、淘宝身份码（淘宝内打开身份码页）、
  菜鸟（**显式 Activity 直达首页**，跳过开屏广告）。
- 点击：立即拉起，不预检不确认；失败给 **Snackbar「去设置」** 兜底（未安装 / 链接打不开 /
  系统拒绝），动作直达设置页——配置失效类错误（如 Activity 改名）能就地修正。
- 长按 chip 弹菜单：**添加到桌面**（`requestPinShortcut`，桌面弹系统确认框）/
...
| 导入 | 我的 → 成绩查询 → 页内「导入」→ 复用 JwImportActivity（成绩模式）→ WebView 打开 `cjcx_frm` → 注入 `fetch` 全部学期成绩 → 确认弹窗（列出学期与条数）→ 按学期替换入库 |
| 展示 | 成绩查询二级页：学期切换（chips，默认最新有数据学期）+ 学期汇总（课程数 / 加权平均分 / 平均绩点）+ 课程卡列表；`pendingReview`（评教未完成被锁定）单独呈现；等级制成绩只展示 `scoreStr` |

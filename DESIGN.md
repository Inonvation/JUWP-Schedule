...
| 净空的消费方 | 课表网格与今日页 dock 自己加；**提示通道 `AppSnackbarHost` 在组件内统一加**（页面底边就是窗口底之后，Snackbar 会落到胶囊与手势条底下，不能让每个调用点自己记着）；课表页的显示设置覆盖面板（`DisplaySettingsContent`）也加在同一处。新增贴住底边的表面时先确认它在这一栏里 |

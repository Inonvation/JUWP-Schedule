...
└── 我的      SettingsScreen（账号条 + 6 入口，2026-09-23 改版）
      ...
      ├── 通用设置   → GeneralSettingsScreen（主题/触感/动态取色/悬浮导航栏/生活页/权限设置）
      ├── 课表       → TimetableHubScreen（副标题「当前：课表名」）
      ├── 学习       → LearningHubScreen（笔记·课件 / 作业）
      ├── 小组件与日历 → WidgetCalendarHubScreen
      ├── 扩展服务   → ExtensionServicesHubScreen（副标题「第三方 · 非学校官方功能」）
...
班级缺失时只剩学号。姓名 / 班级数据源 = **教务导入链路**：成绩接口行里每行都带
`xs0101id`（学号），首次解析成功时按学号匹配 `学籍卡片`（`/jsxsd/grxx/xsxx`）的
`姓名` / `班级`，一并落 DataStore（`profile_name` / `profile_class`）；
学籍卡解析失败**不阻塞**成绩导入，账号条退回纯学号显示。点眼睛图标切换完整学号
仍然只在内存里展示，不写存储不进剪贴板与日志；账号条本体不可点。

**我的**（入口列表，2026-09-21 起 6 分区；2026-09-23 起每个分区收进独立汇总二级页）

- 通用设置（`GENERAL_SETTINGS`，全局观感）：外观主题、动态取色（Material You）、触感反馈、
  悬浮导航栏（§4.22）、生活页开关（§3.13）、权限设置（§3.12）
- 课表（`TIMETABLE_HUB`，副标题「当前：课表名」，组内按「配置 → 使用 → 数据」流排）：
  课表管理 / 课表设置（学期+作息）/ 教务导入 / 上课提醒（§3.7）/ 调课 /
  调课自动检测（§4.17）/ 成绩查询（§4.15）/ 课表数据（导出·导入·清空）
...
- 扩展服务（`EXT_SERVICES_HUB`，副标题标注「第三方 · 非学校官方功能」）：快捷方式（§3.8）/
  快趣出行码（§3.9）/ 水宝宝一卡通（§3.10）/ 胖乖生活一键开水（§3.4）
...
- **底部固定区 = 钉在滚动区下方**（2026-09-19 自「列表尾部两项」改，用户要求底部区置底；
  ...
  - 快捷方式三列图标网格在上（§3.8），**服务格一行两列**在其下，开水卡恒在最底；
    新增快捷方式按序落新行、区块向上生长
  ...
  - **服务格并排**（2026-09-22 自「一卡一行」改，用户拍板）：快趣出行码（§3.9）与水宝宝
    ...
    dock 顶到屏高四成，而每张卡只承载「一行标题 + 一行说明」——并排不丢信息、省约 70dp。
    **开水卡不并**：卡内要放解锁按钮与出水进度
  - **副行可点**（一卡通余额 → 功能入口弹层，2026-09-21 追加）：副行的 `clickable` 嵌在整格
    `clickable` 之内，点它不冒泡触发整格跳转；文案尾部的 `›` 由调用方给（可点性提示）
- **同排高度统一**（2026-09-20 定，2026-09-22 适用面自「四卡」收为「服务格 + 开水卡」）：
    服务格与开水卡（含未登录态）共用最小高度 58dp（两行文本 36dp + 上下 padding 22dp）——
...
| 电量着色 | 低于 `BikeNearby.LOW_BATTERY_HINT_PERCENT`（30%）的百分比标警告色。纯展示阈值，与运营方的 `lowBattery` 无关；电量缺失时不标，缺数据不该变成警告。解析时百分比与运营方阈值都只认 **0~100 的有限数**：接口把「没有数据」写成 `-1`，照收会显示成「-1%」还把它判成电量低（`BikeNearbyTest` 钉住了这条） |
...
`paystep=2` 返回的 `ccctype[0].balance` 为最优先（平台侧实时值）。
...
文案里。2026-09-23 同日补齐：日历读写（课表同步 + 骑行免费时长提醒）与
...
| 区块 | 内容 | 行为 |
|------|------|------|
| 付款码卡 | 标题「付款码」+ 副标题「等同现金，也能刷宿舍门禁」；展开态换主色描边 | 占位条（「点击显示付款码」）→ 点击取码；展开后 QR + Code128 + 「卡号掩码 · 第 N/10 个 · 约 X 小时内有效」+ 换下一个 / 全屏出示 / 收起；凭证未开启时整块给「去开启」 |
| 一卡通余额卡 | 卡账户 + 电子帐户之和 | 点卡片刷新；副行「流水 ›」进 `CAMPUS_STATEMENT` |
| 寝室电费卡 | 房间号 + 剩余电量（度）+ 「≈ ¥X · 0.62 元/度」 | 点卡片刷新；失败保留上次读数并在底行标注「上次 55.57 度 · 20:36」 |
| 常用格 | 一卡通充值 / 电费充值 / 消费流水 / 缴费账单（一行四列） | 充值开 `RechargeSheet`（一卡通未开启时给提示）；后两者开缴费平台网页深链（见下） |
...
**电费充值（一期口径）**：不在 App 内下单，而是把平台页深链交给系统浏览器打开——
`PowerClient.pageUrl` 把 App 现登的 token 放进 URL（前端按 `token` 参数直接登录，与微信
小程序同一口径），落 `/pays?id=181` 房间电费缴费页，支付在网页里完成；「缴费账单」同法落
`/bill`（按月总支出）。App 内下单（`thirdOrder` + 微信直拉）留到二期，181 的单要带房间
场景参数，资金链路需单独实测。

**与既有入口的关系**：今日页的「水宝宝一卡通卡」与「我的 → 校园卡」都保留——前者是
付款码的最短路径（点一下直达），后者管凭证与开关。生活页是它们的汇总视图，不复用同一条
刷新链路（余额走 `CampusCardViewModel`，电费走 `PowerRepository`），但共用同一份凭证与
同一个付款码状态机（`PayCodeViewModel`）。
...
（解析 + `pickRemain` + `roomLabelOf`，纯逻辑）、`PowerRepository.kt`（登录编排 + 读表 +
流水 + 深链），`Graph.powerRepository` 单例；UI 见 §3.13。

测试：`PowerModelsTest`（真实 fixture：项目/读数/流水解析、500 与 401 外壳、剩余电量键回退、
`sceneinfo` 容错、token 与时间解析）、`LifeFeedTest`（混排排序/限量/稳定/时间解析失败沉底）。

#### 电费充值（2026-09-23 实测打通，Python 侧 9 笔真实下单验证）
...
2. UI 弹安全键盘（复用一卡通付款键盘样式），用户点 6 位；**点第 i 位发
   `passwordMap[uuid][i]`**（与服务端样板 `password = numberKeyboard + "$1$" + uuid`
   的口径一致）；
...
**电子账户充值（充电子账户的钱包）与正式卡同链路「立即支付」**：下单通道按目标分
（2026-09-23 实测）——正式卡走 `thirdOrder`（302 收银台）；**电子账户 thirdOrder 会 500，
改走 `/blade-pay/pay`（paystep=0，与电费同通道，JSON 回 orderid）**。支付一步两条路相同：
`paystep=2 + CAMPUSCARD` 直拉微信，**表单必须带 `yktcard`**（正式卡=6位卡号，电子账户=
accinfo type），缺了报「未获取到要充值的卡号」。到账轮询、`PendingRecharge` 状态机两条路
共用（基线 = 付款前该目标账户余额）。

**服务时间**（2026-09-23 夜间实测+用户拍板）：平台**夜间会拒绝支付一步**——同一请求
白天返回 `paysubmit`，夜里一律 `400「未获取到要充值的卡号」`（下单不受影响）。App 处理：
...
**取消未支付订单**：`POST /charge/order/deleteOrder`（带 SIGN）在**官网收银台页**有
「取消订单」按钮，App 侧该接口实测 500（平台行为，DESIGN §4.19 已记），App 内不做取消，
订单 30 分钟自动失效；生活页在下单前查 `GET /charge/order/personal_data?status=0`，
同项目已有未支付订单时先提示（实测同项目未支付订单堆积会导致新下单 500「未知异常」）。

...
- 结构：**焦点卡 + 单时间轴 + 底部固定区（快捷方式网格 → 一键开水卡）**，同一条信息只摆一次
...
- **底部固定区**（恒在列表尾部；开水卡永远最后一项）：
  - 快捷方式三列图标网格在上（§3.8），开水卡在最底；新增快捷方式按序落新行、区块向上生长
  - **一键开水卡恒展示**（默认开，显示设置「显示开水卡片」可关）：已登录保持一键出水交互
...
- 胖乖生活：开水入口 + 开水点击方式（副标题标注「第三方服务 · 非学校官方功能」）
- 关于：版本与免责声明 / 开源仓库
...
| 编辑表单 | 名称必填；目标三种填法：只填包名=打开App / 包名+Activity=直达页面 / 链接=打开链接；包名栏尾按钮**从已安装应用选择**（搜索 + 点选直接填包名，不顺手填 Activity——启动页类名随版本会变）；表单内「测试」即时拉起验证，错误行内提示 |
...
2. **手动检测**：课表页 → 顶栏**导入图标** → 导入弹层里的**「检测课表更新」**条目
   （位置在「从教务导入」下方），点一下就检测：
   无差异 → Snackbar「教务课表无变化」；有差异 → 直接进「更新课表」页。

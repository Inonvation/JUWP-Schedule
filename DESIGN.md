...
├── 课表      WeekScreen
│     ├── 顶栏日期块 → 周次选择弹层
│     ├── 眼睛图标   → 显示设置页内覆盖面板（与「我的 → 显示设置」共用同一份选项内容，见 4.18）
│     └── 课表名 ▾   → 切换课表弹层 → 管理课表
└── 我的      SettingsScreen（入口列表，见 4.17）
      ├── 通用：主题（全局）
...
- 显示设置（眼睛图标）→ **课表页内覆盖面板**（4.18 起；再往前是独立子页 4.17、半屏弹层）：
  `课表字号`（8–14dp，见 3.2）、`教室字号`/`教师字号`（7–14dp，可跟随课名）、
  `时间轴字号`/`日期字号`（7–14dp，**独立于课名**，4.18 拆分）、`格子高度`（50%–150%，乘在自适应行高上，
  默认 110%，超一屏走竖滑，仍受 40dp 下限保护）、`格子圆角`（0–12dp）、`格子不透明度`（50%–100%）；
  开关：`文字水平居中`（默认关=WakeUp 左对齐）、`文字竖直居中`（默认关=顶部起排教师沉底）、
  `显示授课教师`（默认开）、`显示时刻线`（默认开）、`显示周六`/`显示周日`（各自独立，默认均开，4.18 拆分）、
  `地点显示「@」`（默认开，4.18）、`点空白格新建课程`（默认开，4.18）、
...
- 空白格点击：按该节快速加课（可在显示设置关闭防误触）。顶栏不再放「+」（4.18 移除，与导入弹层的
  「手动添加课程」重复）；**课表页不放 FAB**（信息密集页面必然遮挡）
- 「回到本周」（4.18）：仅在当前不在本周时出现的**右下角悬浮按钮**，不占顶栏（放顶栏会在切周时挤动右侧图标）
...
`DefaultData.defaultTimeSlots` 就是上表。作息变更走「我的 → 课表设置 → 作息表」（见 4.17），届时以用户值为准：
...
| C 设置分层 | 归属标注（课表级/全局）、入口收口 | ✅ **已完成**（2026-09-17，见 4.17） | 真机 |

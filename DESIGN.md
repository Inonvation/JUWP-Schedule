...
│     └── 最近流水（一卡通 + 电费混排）→ 全部流水
└── 我的      SettingsScreen（账号条 + 6 入口，2026-09-23 改版）
      ├── 账号条（仅水宝宝一卡通已登录时显示；遮罩学号 + 眼睛切换完整学号；
      │    2026-09-23 升级 = 头像圆标 + 姓名 + 班级副行，数据来自教务导入）
...
账号条（2026-09-23 新增，同日升级为账户卡）：行首 40dp 圆形头像底座 +
`HugeIcons.UserCircle` 向量图标（不落盘任何图片）；标题行 = 姓名（缺省回退遮罩学号，
遮罩口径 = `domain/AccountMask.maskStudentId` 唯一不变）；副行 = 「班级 · 学号遮罩」，
班级缺失时只剩学号。数据源分工（2026-09-23 实测定案）：**姓名**来自水宝宝一卡通
...
账号条自动退回纯学号显示。一卡通接口没有班级字段、头像接口已下线（实测 404），
头像恒用向量图标。点眼睛图标切换完整学号仍然只在内存里展示，
...
| 课程作业列表（`HOMEWORK_COURSE`） | 行 = 勾选框（**勾选即完成，像待办**）+ **正文第一行摘要**（无标题）+ 截止 chip；完成后置灰 + 删除线，不隐藏（可反悔）；点行（非勾选框）进详情 |
...
| 作业中心（`HOMEWORK_TODO`） | 未完成作业汇总（今日卡与截止提醒的落点），**2026-09-23 改为管理页同款折叠分组**（共用 `HomeworkCourseCard`，无「＋」入口）：课程组**默认全展开**（点进来是要处理作业的），手动收起的课程记在内存。组内排序 = 逾期（最近过期在前）→ 今天 → 未来（升序）→ 无截止（创建倒序）；行 = 勾选框（勾上即完成，Snackbar 带「撤销」）+ 正文第一行摘要 + 截止 chip；点行 → 该作业详情。空态给「去作业管理」CTA |
...
| 区块 | 内容 | 行为 |
|------|------|------|
| 付款码卡 | 标题「付款码」+ 副标题「等同现金，也能刷宿舍门禁」；展开态换主色描边 | 占位条（「点击显示付款码」）→ 点击取码；展开后 QR + Code128 + 「卡号掩码 · 第 N/10 个 · 约 X 小时内有效」+ 换下一个 / 全屏出示 / 收起；凭证未开启时整块给「去开启」；**四态同几何固定码位**（2026-09-24 二改，参考出码位口径：Idle/取码中/成功/失败共用 `CodeSlots`——QR 1:1、Code128 6:1 的 `aspectRatio` 占位框，空着是描边空框、取码是呼吸块、出码原地换图；三按钮常显不可用置灰；占位条不再矮一截，任何状态切换零跳动） |
| 一卡通余额卡 | 卡账户 + 电子帐户之和；**与电费卡 `IntrinsicSize.Min` 恒等高**（2026-09-24） | 点卡片刷新；底行改**状态行**（2026-09-24，原固定「点卡片刷新」已删）：有充值确认到账 =「有充值正在确认到账…」、查询中 =「正在查询余额…」、未开启 =「去设置开启」，其余留白；右下角钱包切换图标不变 |
| 寝室电费卡 | 房间号 + 剩余电量（度）+ 「≈ ¥X · 0.62 元/度」 | 点卡片刷新；失败保留上次读数并在底行标注「上次 55.57 度 · 20:36」；底行钉在卡片底（与余额卡同一等高方案） |
...
### 3.15 宿舍报修（2026-09-24，P6；接口与实测见 §4.26）

**入口**：我的 → 扩展服务 → 「学校系统」→ 宿舍报修。

**窗口**：独立 `DormRepairActivity`，锁竖屏，形式与 §3.14 的成绩单导出、§4.4 的教务导入
完全一致。同一个理由：窗口里会随时出现统一身份认证的表单，转屏销毁 WebView 等于让用户
把密码重打一遍。

**一页四件事都在这个窗口里完成**：填表、上传附件、提交、查看流程。App 自己不画表单，
也不代提交——跑的是学工系统官方移动页（§4.26 讲了为什么不做原生）。

**免登**：进窗口即请求学工的统一认证入口，CAS 那边已有会话就直接落到学工首页，
不再问密码。App 里登录过教务导入的用户，第一次进来通常就是已登录状态。
没有会话时就地弹出统一认证登录页，账号与教务是同一套。

**状态条**（顶栏下方一行小字，按当前域切换，别的判断一律不做）：

| 当前域 | 文案 |
|--------|------|
| 统一认证 | 用学校统一身份认证登录（与教务同一个账号） |
| 学工系统 | 已进入学工系统，报修在「宿管服务 → 宿舍报修」 |
| 其他 | 正在打开学工系统… |
...
- **Room v8 → v9（2026-09-23）：`homework` 去 `title`**。SQLite 删列须重建表
  ...
  剥 Markdown 前缀与包边、空行跳过、全空兜底「未命名作业」。这是唯一的摘要口径，
...
| 流水 | `GET /charge/turnover/personal_data?feeitemid=181&flag=3` | 电费充值/退款记录（`abstracts` 带房间标签） |
...
`PowerRepository.kt`（登录编排 + 读表 + 流水 + 深链 + 内存缓存），`Graph.powerRepository`
单例；UI 见 §3.13。
低值提醒（电费低于设定金额）复用同一条读表链路，只是频率为每天一次，规格见 §3.13「余额提醒」。

测试：`PowerModelsTest`（真实 fixture：项目/读数/流水解析、500 与 401 外壳、剩余电量键回退、
`sceneinfo` 容错、token 与时间解析）、`PowerBillTest`（账单：月份按缴费日期归/退回 `feerange`/
...
**流程（生活页）**：电费充值格 → `PowerRechargeSheet`（电子账户余额 + 「仅支持电子账户」
说明 + 取消/去充值/继续）→ 「去充值」打开 `RechargeSheet` 并预选电子账户 →
...
| 余额的强制穿透 | 点余额卡、顶栏刷新、开启流程验证、**到账判定**（`checkArrivalOnce` / `refreshBalanceSilently`）一律 `force = true`。到账判定吃旧值会把「已到账」永远判成「还没到」，这条比省请求重要 |
...
| 位置 | 内容 |
|------|------|
| `data/xg/XgUrls.kt` | 学工地址常量与纯判定（域归属、状态条分档）。不依赖 android.* |
| `ui/campus/DormRepairScreen.kt` | WebView 窗口：状态条 + 进度 + 文件选择 + 返回键接管 |
| `DormRepairActivity.kt` + manifest | 独立窗口，锁竖屏（理由同 §4.4 / §3.14） |
| `ui/me/hub/ExtensionServicesHubScreen.kt` | 入口：「学校系统」分组 |

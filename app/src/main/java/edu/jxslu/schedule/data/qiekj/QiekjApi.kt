...
/**
 * 胖乖生活 Retrofit 接口，端点与参考实现 light-life DeviceApi.kt 一致（DESIGN 4.5）。
 * 全部 Form POST；调用顺序约束见 QiekjRepository.unlockDevice 注释，禁止调整。
 */
interface QiekjApi {
    @FormUrlEncoded
    ...
    @POST("user/reg")
    suspend fun login(
        @Field("channel") channel: String = QiekjApiConfig.LOGIN_CHANNEL,
        @Field("phone") phone: String,
    ...
    suspend fun checkUserIsRisk(@Field("token") token: String): ApiEnvelope<EmptyData>

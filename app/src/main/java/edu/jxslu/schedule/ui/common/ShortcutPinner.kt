...
object ShortcutPinner {
    ...
    fun pin(context: Context, item: ShortcutItem): String? {
        ...
        val intent = when (val plan = edu.jxslu.schedule.domain.Shortcuts.resolveLaunchPlan(item)) {
            is edu.jxslu.schedule.domain.ShortcutLaunchPlan.ExplicitComponent ->
                ShortcutIntents.explicit(plan.pkg, plan.activity)
            is edu.jxslu.schedule.domain.ShortcutLaunchPlan.UriView ->
                ShortcutIntents.uri(plan.uri, plan.pkg)
            is edu.jxslu.schedule.domain.ShortcutLaunchPlan.LaunchPackage ->
